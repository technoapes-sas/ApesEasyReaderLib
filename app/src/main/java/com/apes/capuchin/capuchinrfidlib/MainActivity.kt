...
class MainActivity : AppCompatActivity() {
    ...
    private fun listenToReaderConnected() = runBlocking {
        launch {
            easyReader?.let {
                it.onReaderConnected.collect { onReaderConnected ->
                    val message = when {
                        onReaderConnected -> {
                            configReader()
                            "Reader connected"
    ...
    private fun listenToReaderConnectionFailed() = runBlocking {
        launch {
            easyReader?.let {
                it.onReaderConnectionFailed.collect { abc ->
    ...
    private fun listenToReaderError() = runBlocking {
        launch {
            easyReader?.let {
                it.onReaderError.collect { error ->
    ...
    private fun listenToAntennaPowerChanged() = runBlocking {
        launch {
            easyReader?.let {
                it.onAntennaPowerChanged.collect { power ->
    ...
    private fun listenToAntennaSoundChanged() = runBlocking {
        launch {
            easyReader?.let {
                it.onAntennaPowerChanged.collect { power ->
                    Toast.makeText(applicationContext, power.name, Toast.LENGTH_SHORT).show()
    ...
    private fun listenToSessionControlChanged() = runBlocking {
        launch {
            easyReader?.let {
                it.onSessionControlChanged.collect { session ->
    ...
    private fun listenToStartReading() = runBlocking {
        launch {
            easyReader?.let {
                it.onStartReading.collect { onStart ->
                    val message =
    ...
    private fun listenToItemsRead() = runBlocking {
        launch {
            easyReader?.let {
                it.onItemsRead.collect { inventory ->
                    Toast.makeText(
                        applicationContext,
                        "${inventory.itemsRead.size}",
                        Toast.LENGTH_SHORT
                    ).show()
                }
            }
    ...
    private fun listenToLocateTag() = runBlocking {
        launch {
            easyReader?.let {
                it.onLocateTag.collect { locate ->
    ...
    private fun subscribeToObservable() {
        listenToReaderConnected()
        listenToReaderConnectionFailed()
        listenToReaderError()
        listenToAntennaPowerChanged()
        listenToAntennaSoundChanged()
        listenToSessionControlChanged()
        listenToStartReading()
        listenToItemsRead()
        listenToLocateTag()
    }
}